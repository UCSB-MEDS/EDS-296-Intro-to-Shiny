...
<p class="body-text-s topbr">**1.** In your project repo, create a subdirectory to house your app -- I'm calling mine, `single-file-app`.</p>

. . . 

<p class="body-text-s">**2.** Create a new R script inside `/single-file-app` and name it `app.R` -- you **must** name your script `app.R`. Copy/type the following code into `app.R`, or use the `shinyapp` [snippet](https://rstudio.github.io/rstudio-extensions/rstudio_snippets.html) to automatically generate a shiny app template.<p>
...
<p class="body-text-xs">**Tip:** Use code sections (denoted by `# some text ----`) to make navigating different sections of your app code a bit easier. Code sections will appear in your document outline (find the button at the top right corner of the script/editor panel).</p>

---

##  {#run-app data-menu-title="Run Your App"}

[Run your app]{.slide-title2}
...
[Create a two-file Shiny app]{.slide-title2}
...
<p class="body-text-s">**3.** Lastly, let's create a `global.R` file within `/two-file-app` and add dependencies (right now, that's just loading the `shiny` package). Run your app as we did earlier.</p>
