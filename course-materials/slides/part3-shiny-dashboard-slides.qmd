...
[The [`{shinydashboard}` package](https://rstudio.github.io/shinydashboard/) provides additional UI layout functions that make building apps with a more classic "dashboard" feel a bit easier. You'll always need to import `{shiny}` alongside `{shinydashboard}` (i.e. it's not a full replacement for `{shiny}`).]{.body-text-s}
