...
<p class="body-text-s">[{{< fa check title="check mark" >}}]{.teal-text} applies to any web page (not just shiny apps)</p>
...
[What even is CSS? Sass?]{.slide-title2}
...
[CSS is a rule-based language]{.slide-title2}
...
<p class="body-text-s">You can **(1)** add styling directly to `tags`, **(2)** add CSS rules to your `header`, and/or **(3)** build a stylesheet that is applied to your app. **Creating a stylesheet is often the preferred approach.**</p>
...
[Inspect & identify how to update box styling]{.slide-title3}
...
knitr::include_graphics("images/part4/meds_colors.png")
```
...
knitr::include_graphics("images/part4/sass_to_css_workflow.png")
```
...
[Let's build our Sass file]{.slide-title2}

<hr>

<p class="body-text-s">We'll practice on our `two-file-app` -- first, remove any reference to your `bslib` or `fresh` themes that we practiced applying earlier so that we're starting off with just the default shiny styling.</p>

. . . 

<p class="body-text-s">Next, create your Sass file within `two-file-app/www/`. Then, using your terminal, `cd` into `~/two-file-app/www` and use the `touch` command to create a `.scss` file (I'm calling mine `my-sass-styles.scss`).</p>

. . . 

<p class="body-text-s">Add Sass variables and CSS rules to `my-sass-styles.scss`:</p>

```{css filename="~/two-file-app/www/my-sass-styles.scss"}
#| eval: false
#| echo: true
// import 2 fonts
...
<p class="body-text-s">**Note:** After running your app, you should see a `my-sass-styles.css` file appear in `www/` -- it should look quite familiar, except all of our Sass variables have been converted to CSS.</p>
