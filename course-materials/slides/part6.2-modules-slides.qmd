...
[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} what is a shiny module and when it might make sense to build one]{.body-text-s}

. .  .

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} the structure of a shiny module]{.body-text-s}

. .  .

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} how to use a module]{.body-text-s}
...
[`r fontawesome::fa("box-open", fill = "#05859B", a11y = "sem")` `{gapminder}`: data]{.body-text-s}
...
[This app, developed by [Garrett Grolemund](https://github.com/garrettgman){target="_blank"} & [Joe Cheng](https://github.com/jcheng5){target="_blank"} as part of their [Modules](https://www.rstudio.com/resources/shiny-dev-con/modules/?_ga=2.163786815.1475817156.1670220567-309155161.1666221806){target="_blank"} lesson, taught at the 2016 Shiny Developer Conference, is a prime candidate for modularization.]{.body-text-s}

[It uses the `gapminder` data set to display life expectancy by GDP per capita from 1952 to 2007 for Africa, the Americas, Asia, Europe, Oceania, and all regions collectively. The only difference between each tab is the subset of data displayed.]{.body-text-s}
...
[Taking a closer look at the gapminder app code, we'll see that the following sections of code are repeated for each region (6 times total; only code sections for "all" regions shown below):]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} `tabPanel` (**UI**)]{.body-text-s}
```{r}
#| eval: false
#| echo: true
...
[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} reactive data frame (**server**)]{.body-text-s}
```{r}
#| eval: false
#| echo: true
...
[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} calculating date ranges (**server**)]{.body-text-s}
```{r}
#| eval: false
#| echo: true
...
[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} `renderPlot({})` (**server**)]{.body-text-s}
...
::: {.incremental}
- [**A shiny module is a piece of a shiny app**]{.teal-text} -- it can't be run directly, but instead is included as part of a larger app. While functions work well for code that that is either completely on the client (UI) side or completely on the server side, modules can be written for code that spans both.
...
[**(1)**]{.teal-text} save your modularized code script inside your app's directory (e.g. `~/myApp/myModule.R`)

[If you choose this option, call `source("myModule.R")` into `global.R` (if using `ui.R` / `server.R`) or `app.R`.]{.body-text-s}

<br>

. . . 

[**(2)**]{.teal-text} save your modularized code script inside the `R/` subdirectory of your application (e.g. `~/myApp/R/myModule.R`)
...
[Finally, we can re-write our server. Rather than writing out the *lengthy* code required to make each plot six times over, we can instead call our `gapModuleServer()` function, supplying each call with `id`s that match those used in `gapModuleUI()`, along with the appropriate data subset. Now, run your app! If written correctly, your app should run exactly the same as your initial version.]{.body-text-s}
...
::: {.center-text .body-text-l .teal-text}
Continue your learning journey's with suggested readings and videos on the **[resources page](../../resources.qmd#modules){target="_blank"}**.
:::
