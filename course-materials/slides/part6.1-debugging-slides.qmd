...
[[**(1)**]{.teal-text} commenting out everything except the UI elements and server logic where I believe the issue is stemming from, and]{.body-text-s}
...
[[**(4)**]{.teal-text} Launch reactlog by running `shiny::reactlogShow()` in your console (or use the keyboa)d shortcut `cmd`/`ctrl` + `F3`)]{.body-text-s}
...
<p class="body-text-s">Our `outputId` in the **UI** is set to `penguin_text`:</p>
...
<p class="body-text-s">But we call `penguins_text` (note the plural) when rendering our output in the **server**:</p>
