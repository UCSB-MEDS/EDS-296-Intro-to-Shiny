...
```{r filename="~/scratch/create-fresh-theme-app.R"}
#| eval: false
#| echo: true
# load library ----
library(fresh)
...
##  {#exercise1-prompt data-menu-title="** Exercise 1 Prompt **" background="#d2e3f3"}

[{{< fa question title="question mark" >}} Exercise 1: Use Shiny Assistant to recreate the a `{fresh}` theme for our shinydashboard]{.slide-title3}

<hr>

[**To Do:**]{.body-text-m}

- Use a new Shiny Assistant session to theme our shinydashboard. Generate colors for the following:
  - the primary color (header) 
  - sidebar
  - menuItems 
  - menuItem text 
  - body 
...
- You may want to ask AI to provide the code in it's own separate file, rather than itegrate the theme into your app directly (there are a number of ways to apply `{fresh}` themes, but I find writing the code that generate your `.css` file in it's own, separate script to be the cleanest approach).

::: {.notes}
Generate me a {fresh} theme for a shinydashboard, where:

- the primary color (header) is dark blue
- the sidebar is light blue
- the menuItems are hot pink (when highlighted)
- the menuItem text is red
- the body is light pink

Can you provide me the create_theme() code in it's own separate file (not integrated into my app.R file)
:::
...
[Creating a `{fresh}` theme, by hand, for `shinydashboard`]{.slide-title3}

<hr>

[In this example, we update the colors of our app's header, body, and sidebar using the appropriate `{fresh}` [variables for shiny *dashboards*](https://dreamrs.github.io/fresh/articles/vars-shinydashboard.html){target="_blank"}. We specify a file path, `shinydashboard/www/` where our stylesheet (e.g. `dashboard-fresh-theme.css`, as shown here) file will be saved to. Again, these color combos are *not* recommended.]{.body-text-s}

```{r filename="~/scratch/create-fresh-theme-dashboard.R"}
#| eval: false
#| echo: true
# load libraries ----
library(fresh)
