...
##  {#build-app2 data-menu-title="~~~ Building App #2 ~~~" background="#047C90"}
...
[{{< fa book-open title="an open book" >}} Learning Objectives - App #2 (two-file app)]{.slide-title}

<hr>

[By the end of building out this second app, you should:]{.body-text-m .teal-text}

. . . 

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} be comfortable creating a shiny app using the two-file (`ui.R` & `server.R`) format along with a `global.R` file]{.body-text-s}

. . .

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} understand how to use layout functions to customize the visual structure of your app's UI]{.body-text-s}

. .  .

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} have more practice building reactive outputs -- and placing them within the layout structure of your app]{.body-text-s}

. .  .

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} be able to create multiple inputs that control a given output]{.body-text-s}

. . . 

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} know how to import larger bodies of text using `includeMarkdown()` (rather than writing & styling text within your UI)]{.body-text-s}

. . . 

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} successfully deploy an app using [shinyapps.io](https://www.shinyapps.io/){target="_blank"}]{.body-text-s}

. . .

[Packages introduced:]{.body-text-m .teal-text}

. . .

[[{{< fa box-open title="an open box" >}}]{.teal-text} `{shinyWidgets}`: extend shiny widgets with some different, fun options]{.body-text-s}

. . .

[[{{< fa box-open title="an open box" >}}]{.teal-text} `{lterdatasampler}`: data]{.body-text-s}

---

##  {#roadmap-app2v1 data-menu-title="Roadmap App #2"}

[Roadmap for App #2]{.slide-title2}
...
<br>
<br>
[[**(a)**]{.teal-text} A navigation bar with two pages, one of which will contain two tabs (one tab for each plot)]{.body-text-s}

[[**(b)**]{.teal-text} A `pickerInput` *and* `checkboxGroupButtons` for users to filter cutthroat trout data in a reactive scatter plot]{.body-text-s}

[[**(c)**]{.teal-text} A `pickerInput` for users to filter penguin data *and* a `sliderInput` to adjust the number of bins in a reactive histogram]{.body-text-s}
:::
...
<!-- #| out-width: "100%" -->
<!-- #| fig-align: "center" -->
<!-- #| fig-alt: "RStudio with three columns. (From left to right): (1) a source pane with `ui.R` open (2) a second source pane on the top with `server.R` open and the console on the bottom, (3) Environment/History/Git/etc. on the top and Files/Plots/Packages/etc. on the bottom" -->
<!-- knitr::include_graphics("images/part2/two_source.png") -->
<!-- ``` -->
...
##  {#app2-trout-plot data-menu-title="Draft Plots (App#2, trout)"}

[Practice on your scratch script -- Trout]{.slide-title}

<hr>

In a `~scratch/practice-script-app2.R` file, practice wrangling & plotting trout data. . .

:::: {.columns}

::: {.column width="50%"}
```{r, filename="~/scratch/practice-script-app2.R"}
#| eval: false
#| echo: true
#..........................load packages.........................
...
#| out-width: "100%"
#| fig-align: "center"
#| fig-alt: "A scatterplot with Trout Length (mm) on the x-axis and Trout Weight (g) on the y-axis. Data points are colored and shaped by Channel Type, with data collected from pools represented with green diamonds and data collected from rapids represented by red stars."
knitr::include_graphics("images/part2/trout_plot.png")
```
...
##  {#app2-penguin-plot data-menu-title="Draft Plots (App#2, Penguins)"}

[Practice on your scratch script -- Penguins]{.slide-title}
...
#| out-width: "100%"
#| fig-align: "center"
#| fig-alt: "A histogram of penguin Flipper lengths (mm) with bars colored by species. Adelie are orange and Chinstrap are purple."
knitr::include_graphics("images/part2/penguin_plot.png")
```
...
[We'll use our `global.R` file to help with organization]{.slide-title3}
...
[Example UI layouts (adapted from [Mastering Shiny, Ch. 6](https://mastering-shiny.org/action-layout.html#action-layout){target="_blank"})]{.slide-title3}

<hr>

[**Note:** You can combine multiple layout function groups to really customize your UI -- for example, you can create a navbar, include tabs, and also establish sidebar and main panel areas for inputs and outputs.]{.body-text-s}

::: {.panel-tabset}

## Sidebar & main area

[To create a page with a **side bar and main area** to contain your inputs and outputs (respectively), explore the following layout functions and read up on the [sidebarLayout documentation](https://shiny.rstudio.com/reference/shiny/1.6.0/sidebarlayout){target="_blank"}:]{.body-text-s}
...
knitr::include_graphics("images/part2/sidebar_layout.png")
```
:::
...
[To create a **page with multiple rows**, explore the following layout functions and check out the [fluid layout](https://shiny.rstudio.com/reference/shiny/1.6.0/fluidpage){target="_blank"} documentation. Note that each row is made up of **12** columns. The first argument of the `column()` function takes a value of 1-12 to specify the number of columns to occupy.]{.body-text-s}
...
knitr::include_graphics("images/part2/multirow_layout.png")
```
:::
...
knitr::include_graphics("images/part2/tabsetPanel_layout.png")
```
:::
...
[You may also want to use a navigation bar (`navbarPage()`) with different pages (created using `tabPanel()`) to organize your application. Read through the [navbarPage documentation](https://shiny.rstudio.com/reference/shiny/1.6.0/navbarpage){target="_blank"} and try running the example below:]{.body-text-s}
...
knitr::include_graphics("images/part2/navbarPage_layout.png")
```
:::
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics("images/part2/app2-layout-fxns.png")
```

---

##  {#tips data-menu-title="Tips for success"}

[Important tips for success]{.slide-title}

<hr>

<br>

[{{< fa rainbow title="rainbow" >}}]{.teal-text} **Enable rainbow parentheses**: Tools > Global Options > Code > Display > check Use rainbow parentheses 

<br>

. . . 

[{{< fa comment title="a comment bubble" >}}]{.teal-text} **Add code comments at the start and end of each UI element:** Even with rainbow parentheses, it can be easy to lose track or mistake what parentheses belong with which (see examples on follow slide)

<br>

. . . 

[{{< fa bookmark title="a bookmark" >}}]{.teal-text} **Add placeholder text within your layout functions:** Adding text, like `"slider input will live here"` and `"plot output here"` will help you better visualize what you're working towards. It also makes it easier to navigate large UIs as you begin adding real inputs and outputs.

<br>

. . . 

[{{< fa align-left title="left-aligned text" >}}]{.teal-text} **Use keyboard shortcuts to realign your code:** Select all your code using `cmd` / `ctrl` + `A`, then realign using `cmd` / `ctrl` + `I`.

---

##  {#build-navbar data-menu-title="Build navbar"}

[Build a navbar with two pages]{.slide-title}

<hr>

[First, **let's build a UI that has a navigation bar with two tabs** -- one for background information and one to contain our data visualizations. To do this, we'll use `navbarPage()` instead of `fluidPage()` to create our webpage.]{.body-text-s}
...
[Give your tabs the following titles: `Trout` and `Penguins`.]{.body-text-s}

```{r filename="~/two-file-app/ui.R"}
#| eval: false
#| echo: true
#| code-line-numbers: "15-32"
ui <- navbarPage(
...
[We'll eventually place our input in the sidebar and output in the main panel.]{.body-text-s}
...
##  {#exercise2-prompt data-menu-title="** Exercise 2 Prompt **" background="#d2e3f3"}
...
When you're done, you app should look like this {{< fa arrow-right title="arrow pointing right" >}}
:::
...
##  {#exercise2-solution data-menu-title="** Exercise 2 Solution **" background="#d2e3f3"}
...
[We'll be using the [`and_vertebrates` dataset](https://lter.github.io/lterdatasampler/reference/and_vertebrates.html){target="_blank"} from the [`{lterdatasampler}` package](https://lter.github.io/lterdatasampler/){target="_blank"} to create our first reactive plot. These data contain coastal cutthroat trout (*Oncorhynchus clarkii clarkii*) lengths and weights collected in Mack Creek, [Andrews Forest LTER](https://andrewsforest.oregonstate.edu/){target="_blank"}. Original data can be found on the [EDI Data Portal](https://portal.edirepository.org/nis/mapbrowse?packageid=knb-lter-and.4027.14){target="_blank"}. Refer back to [this slide](part2.2A.qmd#app2-trout-plot){target="_blank"} to revisit our practice data wrangling & visualization script.]{.body-text-s}
...
#| out-width: "100%"
#| fig-align: "center"
#| fig-alt: "A drawing of the right-side profile of a coastal cutthroat trout."
knitr::include_graphics("images/part2/trout.png")
```
...
[Add packages & wrangle data in `global.R`]{.slide-title2}

<hr>

In addition to the [`{lterdatasampler}` package](https://lter.github.io/lterdatasampler/){target="_blank"}, we'll also be using the `{tidyverse}` for data wrangling / visualization, and the [`{shinyWidgets}` package ](https://github.com/dreamRs/shinyWidgets){target="_blank"} to add a `pickerInput` and a `checkboxGroupInput` to our app. 
...
[The `channel_type` variable (originally called `unittype` -- we updated the name when wrangling data) represents the type of water body (*cascade, riffle, isolated pool, pool, rapid, step (small falls), or side channel*) where data were collected. We'll start by building a [`shinyWidgets::pickerInput()`](https://dreamrs.github.io/shinyWidgets/reference/pickerInput.html){target="_blank"} to allow users to filter data based on `channel_type`.]{.body-text-s}

. . . 

[**Reminder:** When we designed our UI layout, we added a `sidebarPanel` to our **Trout** tab with the placeholder text `"trout plot input(s) go here"`. Replace that text with the code for your `pickerInput`:]{.body-text-s}
...
[Save and run your app -- a functional `pickerInput` should now appear in your UI.]{.body-text-s}
...
[Next, we need to create a placeholder in our UI for our trout scatterplot to live. Because we'll be creating a reactive plot, we can use the `plotOutput()` function to do so.]{.body-text-s}

. . . 

[**Reminder:** When we we designed our UI layout, we added a `mainPanel` to our **Trout** tab with the placeholder text `"trout plot output goes here"`. Replace that text with the code for your `plotOuput()`:]{.body-text-s}
...
[Save and run your app -- it won't look different at first glance, but inspecting your app in a browser window (using Chrome, right click > Inspect) will reveal a placeholder box for your plot output to eventually live:]{.body-text-s}
...
#| out-width: "50%"
#| fig-align: "center"
#| fig-alt: "The Trout tab of our app, which contains only a pickerInput with the variables, cascade and pool, selected. There is no visible plot yet, however using Google Chrome's developer tools shows a div (i.e. a box) placeholder where our plot will eventually appear."
knitr::include_graphics("images/part2/trout_plotOutput.png")
```
...
[**Remember the three rules for building reactive outputs:** [**(1)**]{.teal-text} save objects you want to display to `output$<id>`, [**(2)**]{.teal-text} build reactive objects using a `render*()` function, and [**(3)**]{.teal-text} access input values with `input$<id>`:]{.body-text-s}
...
- [If needed, reference your [practice script](part2.2A.qmd#app2-trout-plot){target="_blank"} to remind yourself how you planned to filter and plot your data]{.body-text-s}

- [Reactive data frames need a set of parentheses, `()`, following the name of the df (see `ggplot(trout_filtered_df() ...)`)]{.body-text-s}

- [Saving your ggplot theme as a function in `global.R` makes it easy to build plots with a cohesive appearance (see code on following slide)]{.body-text-s}
:::
...
[Save your custom `ggplot` theme to `global.R`]{.slide-title2}

<hr>

[This allows us to easily add our theme as a layer to each of our ggplots. **Bonus:** If you decide to modify your plot theme, you only have to do so in one place.]{.body-text-s}
...
[Add a second input that will update the same output]{.slide-title3}

<hr>

[You can have more than one input control the same output. Let's now add a `checkboxGroupButtons` widget to our **UI** for selecting forest `section` (either clear cut forest or old growth forest). Check out the [function documentation](https://dreamrs.github.io/shinyWidgets/reference/checkboxGroupButtons.html){target="_blank"} for more information on how to customize the appearance of your buttons.]{.body-text-s}

[Be sure to add the widget to the same `sidebarPanel` as our `pickerInput` (and separate them with a comma!):]{.body-text-s}
...
[Return to your **server** to modify `trout_filtered_df` -- our data frame needs to be updated based on both the `pickerInput`, which selects for `channel_type`, *and* the `checkboxGrouptInput`, which selects for forest `section`:]{.body-text-s}
