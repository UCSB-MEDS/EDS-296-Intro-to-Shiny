...
[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} understand some of the challenges associated with debugging shiny applications]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} know to a few approaches and tools for debugging shiny applications, including using diagnostic messages and the `{reactlog}` package.]{.body-text-s}

. . . 

[Packages introduced:]{.body-text-l .teal-text}

[[{{< fa book-open title="an open book" >}}]{.teal-text} `{reactlog}`:  a reactivity visualizer for shiny]{.body-text-s}
...
- [Shiny is **reactive**, so code execution isn't as linear as other code that you're likely more familiar with (e.g. analytical pipelines written in "normal" R scripts, where each line of code is executed in succession)]{.body-text-s}

- [Shiny code runs behind a web server *and* the Shiny framework itself, which can obscure what's going on]{.body-text-s}
...
- [**isolating pesky errors** (typos, missing commas, unmatched parentheses) in the UI by commenting out code from the outside in]{.body-text-s}

- [**reducing your app to *just* problematic code** by commenting out as much correctly-functioning code as possible]{.body-text-s}

- [**adding diagnostic messages** to reactives]{.body-text-s}

- [**using `{reactlog}`** to visualize reactivity errors]{.body-text-s}

. . . 

We'll touch on each of these, briefly, but be sure to check out the Shiny article, [Debugging Shiny applications](https://shiny.rstudio.com/articles/debugging.html){target="_blank"} and [Mastering Shiny Ch. 5.2](https://mastering-shiny.org/action-workflow.html#debugging){target="_blank"}, by Hadley Wickham, for more approaches, details, and examples.
...
**My preferred approach for troubleshooting a situation like this is to [comment out all code inside the highest-level layout function (e.g. `navbarPage()`)]{.teal-text} > re-run your app to ensure that it works > uncomment the next inner-most pieces > repeat, until you find the place where your app breaks.**
...
[[**(3)**]{.teal-text} ...*and* on rare occasions, I'll try using the [`{reactlog}` package](https://rstudio.github.io/reactlog/){target="_blank} to help visualize my app's reactivity in an attempt to identify the problem.]{.body-text-s}
...
[[**(1)**]{.teal-text} [`working-app`](https://github.com/samanthacsik/EDS430-shiny-app/tree/main/reactlog-working){target="_blank} (a small app that's functioning as intended)]{.body-text-s} 

[[**(2)**]{.teal-text} [`broken-app`](https://github.com/samanthacsik/EDS430-shiny-app/tree/main/reactlog-broken){target="_blank} (the same small app that's *not* functioning as intended)]{.body-text-s}
...
[In **Tab 1**, both the image and text should update whenever a new radio button is chosen. In **Tab 2**, the scatterplot should update so that only data points for penguins with body masses within our chosen range are displayed.]{.body-text-s}
...
[In **Tab 1**, only the image updates whenever a new radio button is chosen, and text is missing altogether. In **Tab 2**, the scatterplot updates as expected whenever the body mass range is changed.]{.body-text-s}
...
server <- function(input, output){
  ...
  output$penguin_img_output <- renderImage({
    ...
    if(input$img_input == "All penguins"){
      ...
      message("Displaying gentoo image")
      list(src = "www/gentoo.jpeg", height = 240, width = 300)
    }
    else if(input$img_input == "Adorable adelie"){
      message("Displaying adelie image")
      list(src = "www/adelie.gif", height = 240, width = 300)
    }
...
[*"A snapshot of the history (**log**) of all **react**ive interactions within a shiny application"* **- Barret Schloerke in his 2019 RSTUDIO::CONF talk, [Reactlog 2.0: Debugging the state of Shiny](https://www.rstudio.com/resources/rstudioconf-2019/reactlog-2-0-debugging-the-state-of-shiny/){target="_blank"}**]{.body-text-s}
:::
...
[Reactivity can be confusing. I recommend watching Barret Schloerke's talk, linked above, and reading through the [Shiny Reactlog vignette](https://rstudio.github.io/reactlog/articles/reactlog.html){target="_blank} as you get started.]{.body-text-s}
...
::: {.footer}
Read about the components of the [status bar](https://rstudio.github.io/reactlog/articles/reactlog.html#status-bar){target="_blank} and the meaning of different [reactive states](https://rstudio.github.io/reactlog/articles/reactlog.html#reactive-states){target="_blank} in the `reactlog` vignette.
:::
...
:::: {.columns}
::: {.column width="50%"}
To visualize the reactive life cycle of the `reactlog-working` app, let's perform the following steps: 

- [load the `{reactlog}` library in our console, by running `library(reactlog)`]{.body-text-s}
- [call `reactlog_enable()` in our console]{.body-text-s}
- [run the app and interact with it -- by default, **All penguins** is selected. For demonstration purposes, let's click down the list (**Sassy chinstrap**, **Staring gentoo**, and finally **Adorable adelie**)]{.body-text-s}
- [when done, stop the app, then run `shiny::reactlogShow()` in the console to open the reactlog visualizer in a browser window]{.body-text-s}
:::
::: {.column width="50%"}
<br>
<br>
...
::: {.footer}
**Note:** We've left the scatter plot on Tab 2 (and it's related UI elements) commented out (as we practiced in the earlier few slides) for this demo -- the `{reactlog}` package has many features that allow you to explore reactive dependencies across your whole app, but it can get complicated quickly. For demo purposes, we're going to work with this "smaller" version of our app, which contains just the problematic code.]{.body-text-s}
:::
...
There's a lot to take when looking at the reactlog viewer, so let's take it one step at a time (click your right arrow key to advance steps in the viewer):
...
[Similar to our functioning app, the default input, **All penguins**, and image output are **Ready** (green). However, in this example our text output is *not a dependency* of our application's input -- there's no linkage and the text output is **Invalidated** (gray).]{.body-text-s}

[As we click down the list of radioButtons, the image output is invalidated, then updated accordingly, but the text output remains disconnected from our input.]{.body-text-s}
:::
...
[Evidence from our diagnostic messages *and* `reactlog` suggests that we should make sure that our UI and server are actually able to communicate about our desired text output. After careful inspection of our `textOutput()` and `renderText()` code, we find that a spelling error is to blame:]{.body-text-s}

[Our `outputId` in the **UI** is set to `penguin_text_output`:]{.body-text-s}
...
[But we call `penguins_text_output` (note the plural) when rendering our output in the **server**:]{.body-text-s}
...
[By updating our `outputId` to match in both the UI and the server, we fix our app.]{.body-text-s}
