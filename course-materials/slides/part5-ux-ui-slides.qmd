...
[Chapters 6](https://engineering-shiny.org/ux-matters.html){target="_blank"} and [7](https://engineering-shiny.org/dont-rush-into-coding.html){target="_blank"} of [Engineering Production-Grade Shiny Apps](https://engineering-shiny.org/index.html){target="_blank"} provide a **list of considerations** as you embark on your app-building journey. A few big ones:

. . . 

- [[**Simplicity is gold:**]{.teal-text} using the application shouldn't require reading a manual, and interfaces should be as self-explanatory as possible.]{.body-text-s}

. . . 

- [[**Adapt a defensive programming mindset:**]{.teal-text} your app should always fail gracefully and informatively (e.g. provide users with a helpful error message)]{.body-text-s}

. . . 

- [[**Build a self-explanatory app:**]{.teal-text} consider the following three suggestions for doing so -- [**(a)**]{.teal-text} remember the rule of least surprise (in UI design, **always do the least surprising thing** e.g. we often assume that underlined text is clickable, so if you include underlined text in your UI, there's a good chance a user will try clicking on it). [**(b)**]{.teal-text} think about progression (**design a clear pattern of moving forward** for your user), and [**(c)**]{.teal-text} related to [**b**]{.teal-text}, make sure that if an input is necessary, it is made clear to your user. Check out the [`{shinyjs}` package](https://deanattali.com/shinyjs/){target="_blank"} for implementing nifty ways to improve the user experience of your shiny apps.]{.body-text-s}

. . . 

- [[**Avoid feature-creep:**]{.teal-text} feature-creep is the process of adding features to an app that complicates its usage and maintenance -- this includes adding too much *reactivity* and too much *interactivity* (e.g. `plotly`) -- interactivity adds visual noise, so it's best to not make elements interactive if there is no value is gained.]{.body-text-s}
...
Check out the **[resources page](../../resources.qmd#ux-ui){target="_blank"}** on the course website for some additional books, videos, and articles on designing effective UIs.
:::
...
[From the [World Wide Web Consortium (W3C)](https://www.w3.org/){target="_blank"}'s [Introduction to Web Accessibility](https://www.w3.org/WAI/fundamentals/accessibility-intro/){target="_blank"}:]{.body-text-s}
...
::: {.footer}
Check out the [A11Y Project](https://www.a11yproject.com/){target="_blank"} for lots of great tutorials and information about web accessibility.
:::
...
[Ensuring that your shiny apps are accessible can feel overwhelming -- but considering even a few small changes can have a large impact on user experience.]{.body-text-s}

[The following suggestions have been borrowed and adapted from [Ch. 6.3 - Web Accessibility](https://engineering-shiny.org/ux-matters.html#web-accessibility){target="_blank"} from [Engineering Production-Grade Shiny Apps](https://engineering-shiny.org/index.html){target="_blank"}, by Colin Fay, Sèbastien Rochette, Vincent Guyader, & Cervan Girard:]{.body-text-s}
...
- [Use HTML elements appropriately (e.g. consider hierarchy)]{.body-text-s}

- [Include alt text for graphical elements]{.body-text-s}

- [Consider navigation from a mobility perspective]{.body-text-s}

- [Use colorblind-friendly palettes]{.body-text-s}
:::
...
Image Source: [Accessibility Stack](https://accessibilityspark.com/9-awesome-web-accessibility-tools/){target="_blank"}
:::
...
[**Screen readers use HTML elements to understand web page organization.** Header elements create hierarchy within a webpage and are used by screen readers (i.e. devices used by those with visual impairments) to understand a page's organizational structure. An `<h1>` element is more important (hierarchically speaking) than an `<h2>`, which is more important than an `<h3>`, and so on.]{.body-text-s}
...
[Include the `alt` argument (similar to adding the `alt` attribute to an HTML element) when using [`renderPlot()`](https://shiny.rstudio.com/reference/shiny/1.7.1/renderplot){target="_blank"} to add alt text to your reactive plots -- the `alt` argument lives *outside* of the `{}` but inside the `()`. For example:]{.body-text-s}
...
[Similarly, use the `alt` argument within `tags$img` when adding static images to your app. For example:]{.body-text-s}
...
A good rule of thumb for writing alt text for data visualizations is <em>alt="<span style="background-color: #FFE700;">Chart type</span> of <span style="background-color: #6DFFF7;">type of data</span> where <span style="background-color: #74FF75;">reason for including chart</span>"</em> (see [this post](https://medium.com/nightingale/writing-alt-text-for-data-visualization-2a218ef43f81#:~:text=Alt%20text%20){target="_blank"} by [Amy Cesal](https://www.amycesal.com/){target="_blank"} for more). One example:
...
<p class="body-text"><em>alt="<span style="background-color: #FFE700;">Bar chart</span> of <span style="background-color: #6DFFF7;">gun murders per 100,000 people</span> where <span style="background-color: #74FF75;">America's murder rate is 6 times worse than Canada, and 30 times Australia</span>"</em></p>

::: {.body-text-s}
For more great tips on how and when to use alt text, check out [this article](https://www.a11yproject.com/posts/alt-text/){target="_blank"} by the A11Y Project. For examples of how to construct good alt text, take a peek at [this resource](https://academy.datawrapper.de/article/330-how-to-write-good-alternative-descriptions-for-your-data-visualization){target="_blank"} by Datawrapper.
:::
...
[For users with mobility impairments, using a mouse to navigate a UI packed with widgets may be challenging -- some users may even be exclusively using a keyboard to navigate the web.]{.body-text-s}

[Ideally, actions required of your user can be done using a keyboard (e.g. pressing a button in the UI) -- however from a new shiny developer standpoint, this may be technically challenging to implement (the authors of Engineering Production-Grade Shiny Apps [suggest](https://engineering-shiny.org/ux-matters.html#c.-navigation){target="_blank"} the [`{nter}` package](https://github.com/JohnCoene/nter){target="_blank"} for building shiny action buttons that can be triggered by pressing `enter`, however, at the time of building this workshop, the package source code hadn't been updated since 2019).]{.body-text-s}
...
[**At a minimum, consider spacing out and / or limiting the number of widgets on any given page to make navigation with a mouse as easy as possible.**]{.body-text-s}
:::
...
[About 1 in 12 males and 1 in 200 females have some form of colorblindness ([Wikipedia](https://en.wikipedia.org/wiki/Color_blindness){target="_blank"}). Ensuring that your color choices are distinguishable from one another and / or providing an additional non-color-based way (e.g. patterns, shapes) of distinguishing between groups is critical.]{.body-text-s}

<!-- can greatly help with interpretation of data visualizations. -->

[There are lots of great colorblind-accessible palettes and resources (check out [this one](https://www.nceas.ucsb.edu/sites/default/files/2022-06/Colorblind%20Safe%20Color%20Schemes.pdf){target="_blank"} by [Alex Phillips](https://www.alexandraatleephillips.com/){target="_blank"}, as well as the [`{paletteer}` package](https://emilhvitfeldt.github.io/paletteer/){target="_blank"}, a near comprehensive collection of all R color palette packages). Using the [Let's get color blind](https://chromewebstore.google.com/detail/lets-get-color-blind/bkdgdianpkfahpkmphgehigalpighjck){target="_blank"} Google Chrome extension can help you quickly assess the accessibility of you visualizations / apps.]{.body-text-s}
