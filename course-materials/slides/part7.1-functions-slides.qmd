...
[[**(2)**]{.teal-text} **made our UI code a bit easier to read** (`penguinSpp_pickerInput()` tells a reader / collaborator / future you *exactly* what that line of code is meant to do, which is create a `pickerInput` that allows users to select penguin species. Even without code comments or additional context, one may deduce what that line of code does)]{.body-text-s}
...
[The goal of this function is to filter the `penguins` data based on the user input *and* render our ggplot scatterplot. To start, I'm going to cut / paste both the code to generate the reactive `filtered_spp_scatterplot` data frame *and* the `renderPlot()` code from `server.R` into our `build_penguin_scatterplot()` function.]{.body-text-s}
...
**Also note that in R, functions return the last executed line** -- when we run `build_penguin_scatterplot()` in our server, it will return the object created by `renderPlot()`.</p>
