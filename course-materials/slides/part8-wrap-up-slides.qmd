...
Image Source: [Reddit](https://www.reddit.com/r/dataengineering/comments/15f9uab/fancy_dashboards_with_volatile_data_pipelines/){target="_blank"}
:::
:::
...
[**[htmlwidgets](https://www.htmlwidgets.org/){target="_blank"} allow you to create interactive data visualizations using JavaScript libraries in R.** They can be embedded into Shiny apps (recall we used both the [`{DT}` package](https://rstudio.github.io/DT/){target="_blank"} to create an interactive datatable and the [`{leaflet}` package](https://rstudio.github.io/leaflet/){target="_blank"} to create an interactive map), *or* into RMarkdown and Quarto Markdown reports. **A well-designed reproducible report with interactive widgets is powerful, easier to maintain, and can be deployed on the web (for free) using tools like [GitHub pages](https://pages.github.com/).** Explore 130 registered widgets on the [htmlwidget gallery](https://gallery.htmlwidgets.org/){target="_blank"}.]{.body-text-s}
...
::: {.center-text .body-text-s .gray-text}
An example leaflet map embedded in an RMarkdown report. Image Source: [htmlwidgets for R](https://www.htmlwidgets.org/index.html){target="_blank"}
:::
...
::: {.center-text .body-text-s .gray-text}
An example leaflet map embedded in a Quarto website. Image Source: [my website](https://samanthacsik.github.io/about.html#fa-hiking-titlea-person-with-a-backpack-and-trekking-pole-the-hobby){target="_blank"}
:::
...
[Have lots of interactive htmlwidgets to present? **Consider composing multiple widgets into a dashboard using [`{flexdashboard}`](https://pkgs.rstudio.com/flexdashboard/){target="_blank"} (based in RMarkdown) or [Quarto dashboard](https://quarto.org/docs/dashboards/){target="_blank"}.** Both frameworks support UI layouts similar to those achieved using `{shiny}` / `{shinydashboard}` (columns, rows, tabsets, navbars, value boxes etc.). You can also optionally use Shiny to embed reactive outputs. Check out the [`{flexdashboard}` gallery](https://pkgs.rstudio.com/flexdashboard/articles/examples.html){target="_blank"} and [Quarto dashboard gallery](https://quarto.org/docs/gallery/#dashboards){target="_blank"} for examples and source code.]{.body-text-s}
...
::: {.center-text .body-text-s .gray-text}
[Waste Lands - America's Forgotten Nuclear Legacy](https://testing-apps.shinyapps.io/flexdashboard-leaflet-waste/){target="_blank"}, (built using `{flexdashboard}`)
:::
...
::: {.center-text .body-text-s .gray-text}
[Recent Earthquakes in Aotearoa New Zealand](https://colorado.posit.co/rsc/nz-quakes/quakes.html){target="_blank"} (built using Quarto)
:::
...
[**Embed reactive shiny components (e.g. a plot with sliders that control its inputs) in Quarto documents.** By using some fun new code chunk options, you can instruct Quarto to spin up it's own self-contained shiny server to run your reactives. Read the [Quarto documentation](https://quarto.org/docs/interactive/shiny/){target="_blank"} to learn more and check out some [teaching examples](https://quarto.org/docs/interactive/shiny/#examples){target="_blank"}.]{.body-text-s}
...
::: {.center-text .body-text-s .gray-text}
[Old Faithful](https://jjallaire.shinyapps.io/shiny-old-faithful/){target="_blank"}, which demonstrates incorporating an interactive plot into the main flow of a document. [Source code](https://github.com/quarto-dev/quarto-web/blob/main/docs/interactive/shiny/_examples/old-faithful/shiny-old-faithful.qmd){target="_blank"}
:::
...
::: {.center-text .body-text-s .gray-text}
[Iris K-Means Clustering](https://jjallaire.shinyapps.io/shiny-k-means/){target="_blank"}, which demonstrates using a more “application-like” page layout (sidebar and main panel). [Source code](https://github.com/quarto-dev/quarto-web/blob/main/docs/interactive/shiny/_examples/k-means/shiny-k-means.qmd){target="_blank"}
:::
...
[**Shinylive is a serverless version of Shiny**, which was recently annoucned at [posit::conf(2023)](https://posit.co/conference/){target="_blank"} by Joe Cheng (rewatch his [talk ](https://www.youtube.com/watch?v=j1M6YyU2ZX8&list=PL9HYL-VRX0oRFZslRGHwHuwea7SvAATHp&index=88){target="_blank"}!). It supports Shiny for both R *and* Python (that's right, you can build shiny apps using Python too! See [Shiny for Python](https://shiny.posit.co/py/){target="_blank"} for more info). You have a few options:]{.body-text-s}
...
- [**Convert shiny apps to static websites** using this [`{shinylive}` package for R](https://github.com/posit-dev/r-shinylive){target="_blank"} or [`{shinylive}` package for Python](https://github.com/posit-dev/py-shinylive){target="_blank"}]{.body-text-s}
- [**Write, run, and share Shiny apps right in the browser** at [https://shinylive.io/r/examples/](https://shinylive.io/r/examples/){target="_blank"} (R) or [https://shinylive.io/py/examples/](https://shinylive.io/py/examples/){target="_blank"} (Python)]{.body-text-s}
- [**Include Shiny apps in Quarto docs** using the [Shinylive Quarto extension](https://github.com/quarto-ext/shinylive){target="_blank"}]{.body-text-s}
:::
...
::: {.center-text .body-text-s .gray-text}
Image Source: [Run a Shiny app in the browser with shinylive for R](https://hypebright.nl/index.php/en/2023/10/02/run-a-shiny-app-in-the-browser-with-shinylive-for-r/){target="_blank"}, by [Veerle van Leemput](https://hypebright.nl/index.php/en/author/veerlevanleemputlive-nl/){target="_blank"}
:::
:::
...
[There are a number of ways to deploy a shiny app, but **stick with [shinyapps.io](https://www.shinyapps.io/){target="_blank"} whenever possible** -- it's the quickest and fastest way to securely host your app. The free tier allows you to host 5 apps with up to 25 active hours a month, however it's easily scalable with a number of paid plan options.]{.body-text-s}
