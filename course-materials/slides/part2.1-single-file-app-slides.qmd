...
[{{< fa book-open title="an open book" >}} Learning Objectives - App #1 (single-file app)]{.slide-title2}

<hr>

[By the end of building out this first app, you should be a bit more familiar with:]{.body-text-m .teal-text}

. .  .

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} writing a single-file (`app.R`) shiny app]{.body-text-s}

. . . 

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} adding and styling text in the UI using `tags`]{.body-text-s}

. . . 

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} practicing data wrangling and visualization *outside* of your shiny app]{.body-text-s}

. . . 

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} following a general workflow for building reactive apps, which includes adding **inputs** and **outputs** to the **UI**, then writing the **server instructions** on how to assemble user input values into outputs]{.body-text-s}

. .  .

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} running and quitting apps in RStudio]{.body-text-s}
...
[[{{< fa box-open title="an open box" >}}]{.teal-text} `{shiny}`: framework for building our reactive app + standard widgets]{.body-text-s}

. . . 

[[{{< fa box-open title="an open box" >}}]{.teal-text} `{DT}`: interactive datatable widgets (that can be made reactive using shiny!)]{.body-text-s}

. . . 

[[{{< fa box-open title="an open box" >}}]{.teal-text} `{tidyverse}`: collection of packages for wrangling & visualizing data]{.body-text-s}

. . . 

[[{{< fa box-open title="an open box" >}}]{.teal-text} `{palmerpenguins}`: data]{.body-text-s}
...
[**(a)**]{.teal-text} A title and subtitle

[**(b)**]{.teal-text} A slider widget for users to select a range of penguin body masses

[**(c)**]{.teal-text} A reactive scatter plot that updates based on user-supplied values
:::
...
`{shiny}` comes with a number of input and output functions, but you can extend these with additional packages (e.g. `{shinyWidgets}`, `{plotly}`, `{DT}`, etc.; more on those later).
...
- [`actionButton()`]{.body-text-s}
- [`checkboxInput()`]{.body-text-s}
- [`checkboxGroupInput()`]{.body-text-s}
- [`dateInput()`]{.body-text-s}
- [`dateRangeInput()`]{.body-text-s}
- [`radioButtons()`]{.body-text-s}
- [`selectInput()`]{.body-text-s}
- [`sliderInput()`]{.body-text-s}
- [`textInput()`]{.body-text-s}
- [See a [full list](https://shiny.rstudio.com/tutorial/written-tutorial/lesson3/){target="_blank"} of `{shiny}` input functions]{.body-text-s}
:::
...
- [`dataTableOutput()` (inserts an interactive table)]{.body-text-s}
- [`imageOutput()` (inserts an image)]{.body-text-s}
- [`plotOutput()` (inserts a plot)]{.body-text-s}
- [`tableOutput()` (inserts a table)]{.body-text-s}
- [`textOutput()` (inserts text)]{.body-text-s}
- [See a [full list](https://shiny.rstudio.com/tutorial/written-tutorial/lesson4/){target="_blank"} of `{shiny}` output functions]{.body-text-s}
:::
...
[Next, we'll create a **scatterplot of penguin bill lengths vs. penguin flipper lengths** using the `penguins` data set from the `{palmerpengiuns}` package. We will make this scatterplot *reactive* by adding a `sliderInput` that allows users to filter the displayed data points by **selecting a range of penguin body masses** (e.g. only plot bill and flipper lengths for penguins with body masses ranging from 4,500 grams to 6,000 grams).]{.body-text-s}

:::: {.columns}

::: {.column width="45%"}
<br>
[**To create a reactive plot, we will follow these steps:**]{.teal-text}

[[**(1)**]{.teal-text} Add an input (e.g. `sliderInput`) to the **UI** that users can interact with]{.body-text-s}

[[**(2)**]{.teal-text} Add an output (e.g. `plotOutput`) to the **UI** that creates a *placeholder space* to fill with our eventual reactive output]{.body-text-s}

[[**(3)**]{.teal-text} Tell the **server** how to assemble inputs into outputs]{.body-text-s}
:::
...
- [Most input functions have a second parameter called `label`, which is used to create a human-readable label for the control, which will appear in the UI.]{.body-text-s}

- [The remaining arguments are unique to each input function. Oftentimes, these include a `value` parameter, which lets you set the default value of your widget, where applicable.]{.body-text-s}
...
::: {.footer}
Check out the interactive [Shiny Widgets Gallery](https://shiny.rstudio.com/gallery/widget-gallery.html){target="_blank"} to learn how to implement the most common widgets.
:::
...
<br>
<br>
When you run your app, you should see something similar to the image to the right. It's operable, but does not yet have an associated output.
:::
...
- Similar to input functions, **all output functions take the same first argument, `outputId`** (again, note `Id` not `ID`), **which connects the front end UI with the back end server.** 
  - For example, if your UI contains an output function with an `outputId = "plot"`, the server function will access it (or in other words, know to place the plot in that particular placeholder) using the syntax `output$plot`.
...
::: {.footer}
To inspect the HTML: open your app in the browser (not necessary, but I find it easier to see what's going on) > right click on your app > Inspect
:::
...
[Now that we've designed our input / output in the UI, we need to write the server instructions (i.e. write the server function) on how to use the input value(s) (i.e. penguin body mass range via a slider input) to update the output (scatter plot).]{.body-text-s}

[The **server function** is defined with two arguments, `input` and `output`, both of which are list-like objects. You *must* define both of these arguments within the server function. `input` contains the values of all the different inputs at any given time, while `output` is where you'll save output objects to be displayed in the app.]{.body-text-s}
...
[**Rules:**]{.body-text-xl .teal-text}

. . . 

[{{< fa angle-right title="a bullet point" >}}]{.teal-text} Save objects you want to display to `output$<id>`

[{{< fa angle-right title="a bullet point" >}}]{.teal-text} Build reactive objects using a `render*()` function

[{{< fa angle-right title="a bullet point" >}}]{.teal-text} Access input values with `input$<id>`
...
server <- function(input, output) {
  ...
  output$body_mass_scatterplot_output <- # code to generate plot here
...
[In our **UI**, we created a placeholder for our plot using the `plotOutput()` function and gave it the Id `"body_mass_scatterplot_output"`.]{.body-text-s} 

[In our **server**, we will save our plot to the output argument by its `outputId`.]{.body-text-s} 

[**Note: In the *UI*, our `outputId` is quoted (`"body_mass_scatterplot_output"`), but not in the *server* (`body_mass_scatterplot_output`).**]{.body-text-s .teal-text}
:::
...
[**Rule 2:** Build reactive objects with `render*()`]{.slide-title3}

<hr>

[Use the appropriate `render*()` function to make your output reactive (e.g. if you have a `plotOutput` in your UI, you will need to use `renderPlot()` in your server). Be sure to include a set of curly brackets, `{}` inside `render*()` -- this allows us to include as many lines of code as necessary to build our object.]{.body-text-s}
...
[I find it easier to experiment and draft my objects (e.g. plots) first in a separate script (*outside* the app directory), then copy the code over to the server after. I want to make a plot that looks like this:]{.body-text-s}
...
[Copy your code over to your app, placing it inside the `{}` (and make sure to add any additional required packages to the top of your `app.R` script). Run your app. What do you notice?]{.body-text-s}

```{r filename="~/one-file-app/app.R"}
#| eval: false
#| echo: true
#| code-line-numbers: "3-4,25-37" 
...
[First, create a new data frame where we filter the `body_mass_g` column for observations within a specific range of values (in this example, values ranging from 3000 - 4000):]{.body-text-s}
...
[Then, plot the new filtered data frame:]{.body-text-s}
...
[Recall that in our **UI**, we gave our `sliderInput()` an `inputId = "body_mass_input"`.]{.body-text-s}
...
[**(1)**]{.teal-text} We created an `app.R` file in it's own directory and began our app with the [template](https://ucsb-meds.github.io/EDS430-Shiny/#/single-file){target="_blank"}, though you can also create a [two-file Shiny app](https://ucsb-meds.github.io/EDS430-Shiny/#/two-file){target="_blank"} by using separate `ui.R` and `server.R` files.

. . .

[**(2)**]{.teal-text} We added an input to the `fluidPage()` in our **UI** using an `*Input()` function and gave it a *unique* `inputId` (e.g. `inputId = "unique_input_Id_name"`).

. . . 

[**(3)**]{.teal-text} We created a placeholder for our reactive object by using an `*Output()` function in the `fluidPage()` of our **UI** and gave it an `outputId` (e.g. `outputId = "output_Id_name"`).

. . .

[**(4)**]{.teal-text} We wrote the **server** instructions for how to assemble inputs into outputs, following these rules:

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} save objects that you want to display to `output$<id>`]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} build reactive objects using a `render*()` function (and similarly, build reactive data frames using `reactive()`]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} access input values with `input$<id>`]{.body-text-s}
...
::: {.column width="50%"}
<br>
[**Working alone or in groups, add a reactive `DT` datatable to your app with a `checkboxGroupInput` that allows users to select which year(s) to include in the table. Configure your `checkboxGroupInput` so that the years 2007 and 2008 are pre-selected.**]{.body-text-s}

[In the end, your app should look something like the example to the right.]{.body-text-s}
:::
...
See next slides for some tips on getting started!
:::

---

##  {#app1-ex1-tips-1 data-menu-title="** Exercise 1 Tips (1/2) **" background="#d2e3f3"}

[`r fontawesome::fa("lightbulb", fill = "#05859B", a11y = "sem")` Exercise 1: Tips (1/2)]{.slide-title}
...
First practice the following in your `practice-script-app1.R` file!
:::

1. There are lots of ways to customize DT tables, but to create a basic one, all you need is `DT::datatable(your_dataframe)`. Do so for the `penguins` data frame.

. . . 

2. Once you have a working `DT::datatable`, practice creating a new data frame that's filtered for just the years `2007` and `2008`. Use this new data frame to create your `DT::datatable`.

. . .

3. Once you've successfully gotten a working data table and understand how to filter for your years of interest, you're ready to move your code over to your `app.R` file. 


---

##  {#app1-ex1-tips-2 data-menu-title="** Exercise 1 Tips (2/2) **" background="#d2e3f3"}

[`r fontawesome::fa("lightbulb", fill = "#05859B", a11y = "sem")` Exercise 1: Tips (2/2)]{.slide-title}

<hr>

<!-- ***And remember to follow the steps / rules, as we've discussed:*** -->

<!-- <br> -->

[**(1)**]{.teal-text} Add an input (e.g. `checkboxGroupInput`) to the **UI** that users can interact with

- [Use `?checkboxGroupInput` to learn more about which arguments you need (remember, all inputs require an `inputId` and oftentimes a `label`, but there are others required to make this work as well)]{.body-text-s}

. . . 

[**(2)**]{.teal-text} Add an output (e.g. `DT::datatableOutput`) to the **UI** that creates a placeholder space to fill with our eventual reactive output

- [Both the `{shiny}` and `{DT}` packages have functions named `dataTableOutput()` and `renderDataTable()`. The `{DT}` versions offer greater flexibility and customization options, so I recommend opting for those variants by namespacing the functions (i.e. using the syntax `packageName::functionName()`).]{.body-text-s}

. . .

[**(3)**]{.teal-text} Tell the **server** how to assemble inputs into outputs following 3 rules:

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} Save objects you want to display to `output$<id>`]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} Build reactive objects using a `render*()` function]{.body-text-s}
...
#| eval: false
#| echo: true
#| code-line-numbers: "5|23-29|62-75"
# load packages ----
library(shiny)
library(tidyverse)
library(palmerpenguins)
...
::: {.column width="50%"}
::: {.incremental}
- [misspelling `inputId` as `inputID` (or `outputId` as `outputID`)]{.body-text-s}

- [misspelling your inputId (or outputId) name in the server (e.g. UI: `inputId = "myInputID"`, server: `input$my_Input_ID`)]{.body-text-s}

- [repeating `inputId`s (each *must* be unique)]{.body-text-s}

- [forgetting to separate UI elements with a comma, `,`]{.body-text-s}

- [forgetting the set of parentheses when calling the name of a reactive data frame (e.g. `ggplot(my_reactive_df(), aes(...))`)]{.body-text-s}
:::
:::
