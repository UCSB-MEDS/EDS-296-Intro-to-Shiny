...
[By the end of this section, you should:]{.body-text-l .teal-text}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} understand the benefits of turning UI elements and server logic into functions]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} know where to write / save your functions]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} successfully turn a repeated input into a function]{.body-text-s}

[[{{< fa angle-right title="a bullet point" >}}]{.teal-text} successfully turn a piece of server logic into a function]{.body-text-s}
...
**Importantly, functions can live outside of your app file(s)** (i.e. `app.R` or `ui.R` / `server.R` / `global.R`), helping you to break up and streamline your code. Hadley Wickham [recommends](https://mastering-shiny.org/scaling-functions.html#file-organisation){target="_blank"} creating a folder called `/R` **inside** your app's directory (e.g. `~/myApp/R/...`) and:

<br>

[[**(a)**]{.teal-text} storing larger functions in their own files (e.g. `~/myApp/R/function-name.R`) and / or]{.body-text-s}

[[**(b)** ]{.teal-text} creating a `utils.R` file (e.g `~/myApp/R/utils.R`) to store smaller, simpler functions all in one script]{.body-text-s}
...
**NOTE:** As of Shiny version 1.5.0, any scripts stored in `~/myApp/R/` will be [**automatically sourced**]{.teal-text} when your application is loaded (meaning you don't *need* to `source()` them into `global.R`, if you're running at least Shiny v1.5.0).
...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel(title = "Scatterplot",
             ...
             pickerInput(inputId = "penguin_spp_scatterplot_input", 
    ...
    tabPanel(title = "Histogram",
             ...
             pickerInput(inputId = "penguin_spp_histogram_input", 
                         label = "Select a species:",
                         choices = c("Adelie", "Chinstrap", "Gentoo"),
                         selected = c("Adelie", "Chinstrap", "Gentoo"),
                         options = pickerOptions(actionsBox = TRUE),
...
[First, create an `R/` folder inside your `functions-app/` directory, then add a new script to this folder. I'm calling mine `penguinSpp-pickerInput.R`.]{.body-text-s}

[Since the only difference between our original two `pickerInput`s are their `inputId`s, we can write a function that takes `inputId` as an argument (**Recall** that `inputId`s must be unique within an app, so it makes sense that both of our `pickerInput`s have different `inputId`s).]{.body-text-s}

[Once written, `source()` your function script into `global.R` (if necessary) to make your function available for use in your app.]{.body-text-s}

::: {.panel-tabset}

## `penguin_spp_pickerInput.R`
```{r filename="~/functions-app/R/penguin_spp_pickerInput.R"}
#| eval: false  
#| echo: true
...
[Finally, replace your original UI code for building both `pickerInput`s with our `penguin_spp_pickerInput()` function, save, and run your app. It should look exactly the same as before!]{.body-text-s}
...
[[**(2)**]{.teal-text} **made our UI code a bit easier to read** (`penguin_spp_pickerInput()` tells a reader / collaborator / future you *exactly* what that line of code is meant to do, which is to create a `pickerInput` that allows users to select penguin species. Even without code comments or additional context, one may deduce what that line of code does)]{.body-text-s}
...
[Next, let's see where we can streamline our server code using functions. We have two discrete sections of code -- [**(1)**]{.teal-text} a reactive data frame and scatterplot output and [**(2)**]{.teal-text} a reactive data frame and histogram output.]{.body-text-s}

```{r filename="~/functions-app/server.R"}
#| eval: false
#| echo: true
#| code-line-numbers: "3-34|36-61"
server <- function(input, output) {
...
[**Remember**, the output of `build_penguin_scatterplot()` is `renderPlot()`, which is used to build our reactive scatterplot. Following our rules for creating reactivity, we need to save our function's output to `output$penguin_scatterplot_output`. In doing so, we reduced 23 lines of code to 1 inside our server function.]{.body-text-s}
```{r filename="~/functions-app/server.R"}
#| eval: false
#| echo: true
...
[We can repeat a similar process to create a function for building our histogram:]{.body-text-s}
...
```{r filename="~/functions-app/R/penguin_spp-pickerInput.R"}
#| eval: false
#| echo: true
penguin_spp_pickerInput <- function(inputId) {
  pickerInput(inputId = inputId, 
