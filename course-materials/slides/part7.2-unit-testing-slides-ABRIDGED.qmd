...
Before we dive into writing any tests, it's super helpful to inspect your app and *importantly*, **jot down (in your own words) the expected behaviors of your application**. These will form the basis of the [assertions](https://en.wikipedia.org/wiki/Test_assertion){target="_blank"} made in your tests.

<br>

. . . 

I find it helpful to make a table that outlines both [**(a)**]{.teal-text} what **actions** can be taken by a user? and [**(b)**]{.teal-text} what are the **expected outputs / behaviors** that result from those actions?
...
It's a pretty annoying error, which seems to be an [ongoing issue](https://github.com/rstudio/chromote/issues/94){target="_blank"} with `{chromote}`. R may be slow to restart (and I've had to restart numerous times {{< fa face-sad-tear title="a sad face with a tear" >}}).
